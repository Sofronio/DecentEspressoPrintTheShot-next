...
- 全量自动化测试通过:7 组、132 项断言

**没有验证的。** 手边没有打印机,也没有 DE1:

- **从未在真实打印机上跑过。** SPP 握手、512 字节分块 + 20ms 间隔、以及「打印机
  能接受不安全 RFCOMM」这个假设,仍然需要真机确认。字节布局有自动化测试覆盖,
  纸面没有。
- **权限流程只在 Android 16 上实测过。** 它们按 API 等级做了正确的分支,用的也都是
  Capacitor 和 Android 的公开文档接口,但没有在 Android 12 / 13 / 14 上跑过。
- **编译状态**:Java 源码在 Java 8 和 Java 17 两个目标下都能干净编译(`-Xlint:all`
  无警告),ESC/POS 常量有一份单元冒烟测试覆盖。
